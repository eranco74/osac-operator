...
var _ = Describe("SecurityGroupReconciler", func() {
	...
	BeforeEach(func() {
		...
		networkClass = &osacv1alpha1.NetworkClass{
			ObjectMeta: metav1.ObjectMeta{Name: "udn-net"},
			Spec: osacv1alpha1.NetworkClassSpec{
				ImplementationStrategy: "udn-net",
			},
	...
	Context("Reconcile", func() {
		...
		It("should lookup NetworkClass from parent", func() {
			...
			Expect(capturedNetworkClass).To(Equal("udn-net"))
		})
		...
		It("should trigger deprovision on delete", func() {
			...
			// Fetch and set deletion timestamp
			...
			// Verify deprovision job was added
			latestJob := osacv1alpha1.FindLatestJobByType(updated.Status.Jobs, osacv1alpha1.JobTypeDeprovision)
			Expect(latestJob).NotTo(BeNil())
		...
		It("should remove finalizer after successful deprovision", func() {
			...
			// Setup deprovision to succeed immediately
			mockProvider.triggerDeprovisionFunc = func(ctx context.Context, resource client.Object) (*provisioning.DeprovisionResult, error) {
				return &provisioning.DeprovisionResult{
					Action:                 provisioning.DeprovisionTriggered,
			...
			// Reconcile deletion - triggers deprovision
			_, err = reconciler.Reconcile(ctx, ctrl.Request{NamespacedName: key})
			Expect(err).NotTo(HaveOccurred())

			// Reconcile again - polls deprovision status and removes finalizer
			_, err = reconciler.Reconcile(ctx, ctrl.Request{NamespacedName: key})
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(updated.Finalizers).NotTo(ContainElement(osacSecurityGroupFinalizer))
		})
	})
	...
	Context("Helper functions", func() {
		It("should append jobs and trim to max history", func() {
			...
			Expect(len(jobs)).To(Equal(reconciler.MaxJobHistory))
